...
Function StopAZVM ($VMObject, $LogFile){  
    ...
    {   
        $Command = { $VmObject | Stop-AzVM -StayProvisioned -Force | Out-Null}
        RunLog-Command -Description $Description -Command $Command -LogFile $LogFile
    }else{
        $Description =  "  -VM in Stopped/deallocated state already - **WARNING VM WILL BE STARTED AS PART OF THE AV SET MIGRATION** "
        RunLog-Command -Description $Description -Command $Command -LogFile $LogFile
    }
}
